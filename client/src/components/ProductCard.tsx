        ...
        >
            ...
            >
                ...
                <div className="relative aspect-[3/4] overflow-hidden bg-surface-2">
                    <img
                        src={product.images[0] || 'https://via.placeholder.com/400x530/1a1a1a/e879f9?text=Zyra'}
                        alt={product.name}
                ...
                {/* Info */}
                <div className="p-4">
                    <p className="text-xs text-gray-500 truncate">{typeof product.category === 'object' ? product.category.name : ''}</p>
                    <h3 className="text-sm font-medium text-white mt-0.5 truncate group-hover:text-brand-300 transition-colors duration-300">{product.name}</h3>

                    <div className="flex items-center gap-1 mt-1.5">
                        <Star size={12} className="text-yellow-400 fill-yellow-400" />
                        <span className="text-xs text-gray-400">{product.averageRating.toFixed(1)} ({product.numReviews})</span>
                    </div>
