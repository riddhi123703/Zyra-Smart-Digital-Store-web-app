...
import { ShoppingBag, Heart, Star, ChevronLeft, ChevronRight, Truck, RefreshCw } from 'lucide-react';
...
export const ProductDetailPage = () => {
    ...
    useEffect(() => {
        api.get(`/products/${slug}`)
            .then(r => { setProduct(r.data.product); setReviews(r.data.reviews); })
    ...
    const handleAddToCart = () => {
        ...
        if (product.colors.length > 0 && !selectedColor) { toast.error('Please select a colour'); return; }
        ...
        if (existing) {
            ...
            toast.success(result === 'updated' ? `Cart updated to ${qty} item${qty > 1 ? 's' : ''}` : 'Added to cart!');
        }
        toggleCart();
    };
    ...
    const inStock = product.stock.length === 0 || product.stock.some(s =>
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        ...
                        <div className="relative aspect-square rounded-3xl overflow-hidden bg-surface-2 group">
                            <AnimatePresence mode="wait">
                                <motion.img
                                    ...
                                    animate={{ opacity: 1 }}
                                    exit={{ opacity: 0 }}
                                    src={product.images[selectedImage] || 'https://via.placeholder.com/600x600/1a1a1a/e879f9?text=Zyra'}
                                    alt={product.name}
                    ...
                    <div className="space-y-6">
                        <div>
                            <p className="text-brand-400 text-sm font-medium">{typeof product.category === 'object' ? product.category.name : ''}</p>
                            <h1 className="text-3xl font-bold text-white mt-1">{product.name}</h1>
                            <div className="flex items-center gap-3 mt-3">
                                <div className="flex items-center gap-1">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} size={16} className={i < Math.round(product.averageRating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'} />
                                    ))}
                                </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {discount > 0 && (
                                <span className="text-brand-400 font-semibold text-sm">{discount}% off</span>
                            )}
                        </div>
                        ...
                        {product.sizes.length > 0 && (
                            <div>
                                <p className="text-sm font-medium text-gray-300 mb-3">
                                    Size {selectedSize && <span className="text-brand-400">— {selectedSize}</span>}
                                </p>
                                <div className="flex flex-wrap gap-2">
                                    {product.sizes.map(s => (
                                        <button
                                            ...
                                            className={`min-w-12 h-11 px-4 rounded-xl border text-sm font-medium transition-all ${selectedSize === s
                        ...
                        {product.colors.length > 0 && (
                            <div>
                                <p className="text-sm font-medium text-gray-300 mb-3">
                                    Colour {selectedColor && <span className="text-brand-400">— {selectedColor}</span>}
                                </p>
                                <div className="flex flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-3 glass rounded-xl px-4 py-3">
                                <button onClick={() => setQty(q => Math.max(1, q - 1))} className="text-gray-400 hover:text-white">−</button>
                                <span className="text-white font-semibold w-6 text-center">{qty}</span>
                                <button onClick={() => setQty(q => q + 1)} className="text-gray-400 hover:text-white">+</button>
                            </div>
                            <button
                            ...
                            >
                                <Heart size={20} className={isWishlisted(product._id) ? 'fill-red-400' : ''} />
                            </button>
                        </div>
                        ...
                        <div className="flex gap-4 pt-2">
                            <div className="flex items-center gap-2 text-xs text-gray-400">
                                <Truck size={14} className="text-brand-400" /> Free shipping above ₹999
                            </div>
                            <div className="flex items-center gap-2 text-xs text-gray-400">
                                <RefreshCw size={14} className="text-brand-400" /> 30-day returns
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-16 grid lg:grid-cols-3 gap-10">
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <form onSubmit={handleReviewSubmit} className="glass rounded-2xl p-6 space-y-4">
                            <div>
                                <label className="text-sm text-gray-400 mb-2 block">Rating</label>
                                <div className="flex gap-1">
                                    {[1, 2, 3, 4, 5].map(n => (
                                        <button key={n} type="button" onClick={() => setReviewForm(f => ({ ...f, rating: n }))}>
                                            <Star size={24} className={n <= reviewForm.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'} />
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                {reviews.map(r => (
                                    <div key={r._id} className="glass rounded-2xl p-5">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex gap-0.5 mt-0.5">
                                                        {[...Array(5)].map((_, i) => (
                                                            <Star key={i} size={12} className={i < r.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'} />
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <h4 className="text-sm font-semibold text-white mt-3">{r.title}</h4>
                                        <p className="text-sm text-gray-400 mt-1 leading-relaxed">{r.body}</p>
                                        {r.isVerifiedPurchase && (
                                            <span className="inline-block mt-2 text-xs text-green-400 glass px-2 py-0.5 rounded">✓ Verified Purchase</span>
                                        )}
                                    </div>
                                ))}
                            </div>
