...
export const AdminOrders = () => {
  ...
  const updateStatus = async (orderId: string, status: string) => {
    ...
    try {
      ...
      setOrders(prev =>
        prev.map(o => (o._id === orderId ? { ...o, orderStatus: status as Order['orderStatus'] } : o))
      );
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="relative inline-block">
                      ...
                      >
                        {STATUSES.map(s => (
                          <option key={s} value={s} className="bg-surface-2 text-white capitalize">
                            {s.charAt(0).toUpperCase() + s.slice(1)}
                          </option>
                        ))}
                      </select>
                      <ChevronDown size={10} className="input-icon-right pointer-events-none opacity-60" />
                    </div>
                  </td>
