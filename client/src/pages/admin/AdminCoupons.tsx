...
import { Copy, Loader2, Plus, RefreshCw, TicketPercent, Trash2, X } from 'lucide-react';
...
export const AdminCoupons = () => {
    ...
    const stats = useMemo(() => {
        return {
            ...
            expired: coupons.filter((coupon) => coupon.expiresAt && new Date(coupon.expiresAt) < new Date()).length,
        };
    }, [coupons]);  
        ...
        <div className="fade-in">
            ...
            <div className="grid gap-4 sm:grid-cols-3 mb-8">
                ...
                <div className="glass rounded-2xl p-4">
                    ...
                    <p className="text-2xl font-bold text-brand-300 mt-1">{stats.active}</p>
                </div>
                <div className="glass rounded-2xl p-4">
                    <p className="text-xs text-gray-400">Expired coupons</p>
                    <p className="text-2xl font-bold text-red-300 mt-1">{stats.expired}</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="glass rounded-2xl p-10 text-center">
                    ...
                    <p className="text-gray-500 text-sm mt-1">Generate your first coupon from the admin UI.</p>
                </div>
            ) : (
                <div className="grid gap-4 lg:grid-cols-2">
                    {coupons.map((coupon) => {
                        ...
                        return (
                            <div key={coupon._id} className="glass rounded-2xl p-5 border border-white/5">
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <h3 className="text-lg font-bold text-white tracking-wide">{coupon.code}</h3>
                                            <button onClick={() => copyCode(coupon.code)} className="text-gray-400 hover:text-white transition-colors">
                                                <Copy size={14} />
                                            </button>
                                            <span className={`text-xs px-2 py-1 rounded-full ${coupon.isActive ? 'bg-brand-500/20 text-brand-300' : 'bg-white/10 text-gray-300'}`}>
                                        ...
                                        <p className="text-sm text-gray-400 mt-2">
                                            {coupon.type === 'percent' ? `${coupon.value}% off` : `₹${coupon.value} off`} · Min order ₹{coupon.minOrderAmount.toLocaleString()}
                                        </p>
                                    ...
                                    >
                                        {deletingId === coupon._id ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                                    </button>
                                </div>
                            </div>
                        );
                            ...
                            >
                                <div className="flex items-center justify-between p-6 border-b border-white/10">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 mt-1">Use the form to create a backend coupon code.</p>
                                    </div>
                                ...
                                <form onSubmit={handleSave} className="p-6 space-y-3">
                                    <div>
                                        <label className="block text-xs font-medium text-gray-300 mb-1">Coupon Code</label>
                                        <div className="flex gap-2">
                                            ...
                                            />
                                            <button type="button" onClick={regenerateCode} className="btn-outline py-2 px-3 text-sm">
                                                <RefreshCw size={14} />
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="block text-xs font-medium text-gray-300 mb-1">Discount Type</label>
                                            <select
                                                value={form.type}
                                    ...
                                    <div className="flex gap-3 pt-2">
                                        ...
                                        <button type="submit" disabled={saving} className="btn-primary flex-1 justify-center py-2.5 text-sm">
                                            {saving ? <Loader2 size={16} className="animate-spin" /> : 'Create Coupon'}
                                        </button>
                                    </div>
                                </form>
                            </motion.div>
